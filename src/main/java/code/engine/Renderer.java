...
public class Renderer {
	...
	private ArrayList<ImageRequest> imgReq = new ArrayList<ImageRequest>();
	private ArrayList<LightRequest> lgtReq = new ArrayList<LightRequest>();

	// Ancho y alto del pixel
	private int pW;
	private int pH;
	...
	// Luz del bloque
	private int[] lgtBlock;

	// Color de ambiente y profundidad de la capa
	private int ambientColor = 0xff6b6b6b;
	private int zDepth = 0;

	private boolean processing = false;

	// Valores en coordenadas XY de la camara
	private int camX;
	private int camY;
	...
	public Renderer(GameEngine ge) {
		pW = ge.getWidth();
		pH = ge.getHeight();
		pixel = ((DataBufferInt) ge.getWindow().getImage().getRaster().getDataBuffer()).getData();
		zBuffer = new int[pixel.length];
		lgtMap = new int[pixel.length];
		lgtBlock = new int[pixel.length];
	}
	...
	public void process() {
		...
		Collections.sort(imgReq, new Comparator<ImageRequest>() {
			@Override
		...
		// Luz
		for (int i = 0; i < lgtReq.size(); i++) {
			LightRequest l = lgtReq.get(i);
			drawLightRequest(l.light, l.locX, l.locY);
		}

		for (int i = 0; i < pixel.length; i++) {
			float r = ((lgtMap[i] >> 16) & 0xff) / 255f;
			float g = ((lgtMap[i] >> 8) & 0xff) / 255f;
			float b = (lgtMap[i] & 0xff) / 255f;

			pixel[i] = ((int) (((pixel[i] >> 16) & 0xff) * r) << 16
					| (int) (((pixel[i] >> 8) & 0xff) * g) << 8
					| (int) ((pixel[i] & 0xff) * b));
		}
	...
	public void setLightMap(int x, int y, int value) {
		...
		int baseColor = lgtMap[x + y * pW];
		int maxRed = Math.max((baseColor >> 16) & 0xff, (value >> 16) & 0xff);
		int maxGreen = Math.max((baseColor >> 8) & 0xff, (value >> 8) & 0xff);
		int maxBlue = Math.max(baseColor & 0xff, value & 0xff);

		lgtMap[x + y * pW] = (maxRed << 16 | maxGreen << 8 | maxBlue);
	}
	...
	public void drawImage(Image image, int offX, int offY) {
		...
		if (image.isAlpha() && !processing) {
			imgReq.add(new ImageRequest(image, zDepth, offX, offY));
			return;
		}
	...
	public void drawLight(Light l, int offX, int offY) {
		lgtReq.add(new LightRequest(l, offX, offY));
	}
	...
	private void drawLightLine(Light l, int x0, int y0, int x1, int y1, int offX, int offY) {
		...
		while (true) {
			...
			if (lgtBlock[screenX + screenY * pW] == Light.FULL) {
				return;
			}
